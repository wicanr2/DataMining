...
public class DecisionTreeDriver {
	...
	public void makeDecisionNode(DecisionNode node) {
		...
		for ( int k = 0 ; k < node.getCategoryList().size() ; k++ ) {
			...
			for(int i = 0 ; i < tmpC.getRangeSize() ;i++ ) {
				...
				sb1.append(tmpR.getInformation(i)).append("\t");
		...
		for ( int i = 0 ; i <  infoList.size() ; i++ ) {
			if ( maxInfo == null ) {
				...
				if ( infoList.get(i).getGainRatio() > maxInfo.getGainRatio() ) {
					maxInfo = infoList.get(i);
				}
			}
		}
        ...
        node.setGainRatio(maxInfo.getGainRatio());
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("WJY decision tree C4.5 algorithm");
